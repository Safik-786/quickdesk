...
export default function Dropdown({
  ...
  return (
    <div className={`w-full relative ${className}`} ref={dropdownRef}>
      ...
      <div className={`bg-blue-50 p-1 rounded-xl ${wrapperClassName}`}>
        ...
        >
          ...
          <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none text-gray-400">
            <svg className={`transition-transform duration-200 ${isOpen ? 'rotate-180' : ''} ${iconClassName || 'h-4 w-4'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {isOpen && (
        <div className={`absolute z-10 mt-1 border border-slate-200 w-full bg-white shadow max-h-60 rounded-lg py-1 text-[10px] md:text-xs ring-opacity-5 overflow-auto focus:outline-none ${menuClassName}`}>
          {options.length === 0 ? (
            ...
            options.map((option) => (
              <div
                ...
                className={`
                  cursor-pointer select-none relative py-1 pl-1.5 pr-5 hover:bg-blue-50 hover:text-blue-900 transition-colors
              ...
              >
                ...
                {option.value === value && (
                  <span className="absolute inset-y-0 right-0 flex items-center pr-1.5 text-blue-600">
